    ...
    <main class="flex-1 max-w-4xl mx-auto p-6 w-full">
      ...
      <div class="flex gap-2 mb-6">
        <button
          ...
          :class="activeTab === 'rules' ? 'bg-white shadow-lg text-orange-600' : 'bg-white/50 text-gray-500 hover:bg-white/70'"
        >
          📋 规则管理
        </button>
        <button
          ...
          class="px-5 py-2.5 rounded-xl font-medium transition-all"
          :class="activeTab === 'tags' ? 'bg-white shadow-lg text-orange-600' : 'bg-white/50 text-gray-500 hover:bg-white/70'"
        >
          🏷️ 标签管理
        </button>
      </div>
                ...
                >
                  ...
                  <template v-if="editingRule?.id === rule.id">
                    ...
                    <div class="flex gap-1 ml-3">
                      <button @click="saveEditRule" class="text-green-600 hover:text-green-700 px-3 py-1.5 font-medium text-sm bg-green-100 rounded-lg hover:bg-green-200 transition-colors">保存</button>
                      <button @click="cancelEditRule" class="text-gray-500 hover:text-gray-700 px-3 py-1.5 font-medium text-sm bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">取消</button>
                    </div>
                  </template>
      ...
      <template v-if="activeTab === 'tags'">
            ...
            <div class="flex items-center gap-2">
              ...
              <div class="flex gap-1.5">
                <button
                  ...
                  :class="newTagColor === color ? 'ring-2 ring-offset-2 ring-gray-400' : ''"
        ...
        <!-- 标签列表 -->
        <div class="bg-white rounded-2xl p-6 shadow-lg" v-if="tags.length > 0">
          ...
          <div class="flex flex-wrap gap-3">
            ...
            >
              ...
              <template v-if="editingTag?.id === tag.id">
                <div class="flex items-center gap-2 p-2 bg-gray-50 rounded-xl border-2 border-orange-300">
                  <input
                    v-model="editTagName"
                    type="text"
                  ...
                  <div class="flex gap-1">
                    <button
                      ...
                      :class="editTagColor === color ? 'ring-2 ring-offset-1 ring-gray-400' : ''"
                      :style="{ backgroundColor: color }"
                    />
                  </div>
                  <button @click="saveEditTag" class="text-green-600 hover:text-green-700 px-2 py-1 text-sm font-medium">保存</button>
                  <button @click="cancelEditTag" class="text-gray-500 hover:text-gray-700 px-2 py-1 text-sm font-medium">取消</button>
                </div>
              </template>
              ...
              <template v-else>
                <div
                  class="px-4 py-2 rounded-full text-white font-medium flex items-center gap-2 cursor-default"
                  :style="{ backgroundColor: tag.color }"
                >
                  {{ tag.name }}
                  <button
                    @click="startEditTag(tag)"
                    class="opacity-0 group-hover:opacity-100 transition-opacity text-white/70 hover:text-white"
                  >
                    ✏️
                  </button>
                  <button
        ...
        <div v-if="tags.length === 0" class="text-center py-12 text-gray-500 bg-white rounded-2xl shadow-lg">
